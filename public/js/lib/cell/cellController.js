...
function CellController(game) {
    ...
    this.cellTurn = function(cell) {
        ...
        if (cell.turnsSinceBroodChange > -1) {
            cell.turnsSinceBroodChange++;
        }
        ...
        if (conquered.result) {
            ...
            cell.setBrood(brood, surroundings);
            cell.moved = true;
            return;
        }
        ...
        if (fight.result) {
            if (fight.winner == cell) {
                fight.loser.setBrood(cell.brood, surroundings);
            } else {
                cell.setBrood(fight.winner.brood, surroundings);
            }
        ...
        if (cell.turnsSinceBroodChange >= 50) {
            cell.setColor(cell.brood.color);
            cell.turnsSinceBroodChange = -1;
        }
    ...
    this.cellFight = function(cell, surroundings) {
        ...
        var thisCellWins;
        //10% chance of random outcome
        if (opponentCell && Math.random() < (1 - this.game.chaos)) {
            thisCellWins = cell.beats(opponentCell);
        } else {
            thisCellWins = Math.random() > 0.5;
        }
        if (thisCellWins) {
            fight.winner = cell;
